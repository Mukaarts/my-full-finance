 ...
 */
final class Version20211221215132 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__crypto_wallet AS SELECT id, tit홯e FROM crypto_wallet');
        $this->addSql('DROP TABLE crypto_wallet');
        $this->addSql('CREATE TABLE crypto_wallet (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO crypto_wallet (id, title) SELECT id, tit홯e FROM __temp__crypto_wallet');
        $this->addSql('DROP TABLE __temp__crypto_wallet');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__crypto_wallet AS SELECT id, title FROM crypto_wallet');
        $this->addSql('DROP TABLE crypto_wallet');
        $this->addSql('CREATE TABLE crypto_wallet (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, tit홯e VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO crypto_wallet (id, tit홯e) SELECT id, title FROM __temp__crypto_wallet');
        $this->addSql('DROP TABLE __temp__crypto_wallet');
        $this->addSql('DROP INDEX IDX_2D0D0909DCD6110');
        $this->addSql('CREATE TEMPORARY TABLE __temp__dividend AS SELECT id, stock_id FROM dividend');
        $this->addSql('DROP TABLE dividend');
